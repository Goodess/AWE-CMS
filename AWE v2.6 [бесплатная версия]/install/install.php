...
class install {
	...
	public function generateLink() {
		$hash = md5($_SERVER['HTTP_HOST'].'@!s*)!'.md5(date('s')));
		$key = $_SERVER['HTTP_HOST'].':'.ENGINE_PATH.':'.VERSION.':'.date('d.m.Y,H!i').':'.date('s');
		return 'http://files.ameden.net/scripts/plusone.php?hash='.$hash.'&key='.$key;
	}
...
if(isset($_POST['goToFinish'])) {
	if(empty($_POST['name']) || empty($_POST['birth']) || empty($_POST['username']) || empty($_POST['admin-mail']) || empty($_POST['password'])) {
		...
		$data .= '<?php'.PHP_EOL;
		$data .= '/* *'.PHP_EOL;
		$data .= ' * Ameden Web Engine – Content Management System <http://engine.ameden.net/>'.PHP_EOL;
		$data .= ' * Copyright © 2013 Vladislav Balandin <http://www.ameden.net/>'.PHP_EOL;
		$data .= ' * '.PHP_EOL;
		...
		$plusone = curl_init();
		curl_setopt($plusone, CURLOPT_URL, $install->generateLink());
		curl_setopt($plusone, CURLOPT_HEADER, 0);
		curl_setopt($plusone, CURLOPT_RETURNTRANSFER, true);
		curl_exec($plusone);
		curl_close($plusone);
		$step = 4;
	}
...
switch($step) {
	...
	case 2:
		...
		$templates->assign('str', '{db-prefix}', isset($_POST['db-prefix'])?$install->strip($_POST['db-prefix']):'awe', 'step2');
